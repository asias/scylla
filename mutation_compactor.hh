...
class compact_mutation_state {
    ...
    std::function<gc_clock::time_point(const dht::decorated_key&, const gc_clock::time_point& query_time)> _get_gc_before;
    can_gc_fn _can_gc;
...
private:
    ...
    gc_clock::time_point get_gc_before() {
        if (_gc_before) {
            ...
            if (_dk && _get_gc_before) {
                _gc_before = _get_gc_before(*_dk, _query_time);
                return _gc_before.value();
            } else {
...
public:
    ...
    compact_mutation_state(const schema& s, gc_clock::time_point compaction_time,
        ...
        , _get_gc_before(std::move(get_gc_before))
...
public:
    ...
    compact_mutation(const schema& s, gc_clock::time_point compaction_time,
        ...
        : _state(make_lw_shared<compact_mutation_state<OnlyLive, SSTableCompaction>>(s, compaction_time, get_max_purgeable, get_gc_before))
        , _consumer(std::move(consumer))
        , _gc_consumer(std::move(gc_consumer)) {
    }
