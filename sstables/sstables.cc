...
// sstable is worth dropping some tombstones. We only return the minimum
...
// record.
gc_clock::time_point sstable::get_gc_before_for_drop_estimation(const gc_clock::time_point& query_time) const {
    auto s = get_schema();
    ...
    sstlog.info("sstable={}, ks={}, cf={}, range={}, estimate", get_filename(), s->ks_name(), s->cf_name(), range);
    return s->get_gc_before(range, query_time).first;
}
...
gc_clock::time_point sstable::get_gc_before_for_fully_expire(const gc_clock::time_point& query_time) const {
    ...
    if (deletion_time == gc_clock::time_point(gc_clock::duration(std::numeric_limits<int>::max()))) {
        sstlog.info("sstable={}, ks={}, cf={}, get_max_local_deletion_time={}, min_timestamp={}, gc_grace_seconds={}, shortcut",
    ...
    auto end = get_last_decorated_key().token();
    auto range = dht::token_range(dht::token_range::bound(start, true), dht::token_range::bound(end, true));
    sstlog.info("sstable={}, ks={}, cf={}, range={}, get_max_local_deletion_time={}, min_timestamp={}, gc_grace_seconds={}, query",
            get_filename(), s->ks_name(), s->cf_name(), range, deletion_time, get_stats_metadata().min_timestamp, s->gc_grace_seconds().count());
    auto p = s->get_gc_before(range, query_time);
    auto gc_before = p.first;
    auto knows_entire_range = p.second;
    if (knows_entire_range) {
        return gc_before;
    }
    return knows_entire_range ? gc_before : gc_clock::time_point::min();
}
