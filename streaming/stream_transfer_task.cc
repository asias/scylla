...
future<> send_mutation_fragments(lw_shared_ptr<send_info> si) {
  return si->estimate_partitions().then([si] (size_t estimated_partitions) {
    ...
    return netw::get_local_messaging_service().make_sink_and_source_for_stream_mutation_fragments(si->reader.schema()->version(), si->plan_id, si->cf_id, estimated_partitions, si->reason, si->id).then([si] (rpc::sink<frozen_mutation_fragment, bool> sink, rpc::source<int32_t> source) mutable {
        ...
        auto sink_op = [sink, si, got_error_from_peer] () mutable -> future<> {
            return do_with(std::move(sink), [si, got_error_from_peer] (rpc::sink<frozen_mutation_fragment, bool>& sink) {
                return repeat([&sink, si, got_error_from_peer] () mutable {
                    return si->reader(db::no_timeout).then([&sink, si, s = si->reader.schema(), got_error_from_peer] (mutation_fragment_opt mf) mutable {
                        if (mf && !(*got_error_from_peer)) {
                            ...
                            bool error = false;
                            return sink(fmf, error).then([] { return stop_iteration::no; });
                        } else {
                            return make_ready_future<stop_iteration>(stop_iteration::yes);
                        }
                    });
                    ...
                    bool error = true;
                    return sink(frozen_mutation_fragment(bytes_ostream()), error).then([ep = std::move(ep)] () mutable {
