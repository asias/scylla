...
future<> stream_blob_handler(replica::database& db, netw::messaging_service& ms,
    ...
    try {
        ...
        if (it != sstring::npos) {
            auto data_path = std::filesystem::path(dst_filename);
            blogger.info("fstream[{}] Started loading sst {}", meta.ops_id, dst_filename);
            auto& table = db.find_column_family(meta.table);
            auto desc = sstables::entry_descriptor::make_descriptor(data_path, table.schema()->ks_name(), table.schema()->cf_name());
#if 0
