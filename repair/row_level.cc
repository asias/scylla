...
private:
    ...
    future<> prepare_sstables_for_incremental_repair() {
        ...
        auto states = co_await table.get_table_states_for_range(_range);
        for (auto ts : states) {
            auto cre =  co_await _db.local().get_compaction_manager().await_unrepaired_and_disable_compaction(*ts);
            ...
            auto lock_holder = co_await _db.local().get_compaction_manager().get_incremental_repair_write_lock(*ts, "row_level_repair");
        ...
        for (auto& snap : sstables) {
            ...
            if (repair::is_repaired(sstables_repaired_at, sst)) {
                ...
                sst->being_repaired = _frozen_topology_guard;
...
future<> repair_service::init_ms_handlers() {
    ...
    ser::repair_rpc_verbs::register_repair_compaction_ctrl(&ms, [this] (const rpc::client_info& cinfo, locator::global_tablet_id gid, service::frozen_topology_guard topo_guard) -> future<> {
        co_await container().invoke_on_all([gid, topo_guard] (repair_service& local_repair) mutable -> future<> {
            ...
            rlogger.info("Got repair_compaction_ctrl gid={} session_id={} removed={}", gid, topo_guard, removed);
        });
    });

    ser::repair_rpc_verbs::register_repair_update_repaired_at(&ms, [this] (const rpc::client_info& cinfo, table_id table) -> future<> {
        rlogger.debug("Got repair_update_repaired_at for table={}", table);
