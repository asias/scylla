...
void storage_service::handle_state_normal(inet_address endpoint) {
    ...
    if (_gossiper.uses_host_id(endpoint)) {
        ...
        if (db().local().is_replacing() &&
            ...
            // This local node is the replacing node.
            // There are two possible cases:
            // 1) the replacing node becomes NORMAL status
            // 2) the node being replaced becomes NORMAL status
            ...
            if (endpoint == get_broadcast_address()) {
                ...
                if (*replace_addr != get_broadcast_address()) {
                    // The replacing node has the different ip address as the node being replaced
                   slogger.info("Remove the node to be replaced={} from token_metadata because the replacing node={} has become NORMAL status",
                    ...
                    do_remove_node(*replace_addr);
                } else {
                    // The replacing node has the same ip address as the node being replaced
                    slogger.info("Remove node {} from pending replacing endpoint", endpoint);
                    _token_metadata.del_replacing_endpoint(endpoint);
                }
                // We make the replacing node to own the host_id here and own
                // the tokens below. However, since we already call
                // token_metadata.update_normal_tokens inside join_token_ring
                // after bootstrap is done the tokens will be updated to
                // be owned the replacing node already.
                slogger.info("Set host_id={} to be owned by node={}, case0", host_id, endpoint);
                _token_metadata.update_host_id(host_id, endpoint);
            } else {
                // The node being replaced becomes NORMAL status. The replacing
                // node, this local node, learns the replaced node status from
                // other nodes in the cluster. We make the node being replaced
                // to own the host_id here and own the tokens below.
                slogger.info("Set host_id={} to be owned by node={}, case1", host_id, endpoint);
                _token_metadata.update_host_id(host_id, endpoint);
            }
        } else {
            if (existing && *existing != endpoint) {
                if (*existing == get_broadcast_address()) {
                    // Existing node is the node itself
                    ...
                    slogger.info("Set host_id={} to be owned by node={}, case2", host_id, endpoint);
                    _token_metadata.update_host_id(host_id, endpoint);
                } else {
                ...
                // Existing node and the node becomes NORMAL status are the same node.
                // The replacing node has the same ip address as the node being replaced
                slogger.info("Set host_id={} to be owned by node={}, case3", host_id, endpoint);
                _token_metadata.update_host_id(host_id, endpoint);
                slogger.info("Remove node {} from pending replacing endpoint", endpoint);
                _token_metadata.del_replacing_endpoint(endpoint);
            } else {
                // No existing node
                slogger.info("Set host_id={} to be owned by node={}, case4", host_id, endpoint);
                _token_metadata.update_host_id(host_id, endpoint);
            }
        }
    }
