    ...
    future<> handle_tablet_resize_finalization(group0_guard g) {
        ...
        for (auto& table_id : plan.resize_plan().finalize_resize) {
            ...
            if (old_cnt > new_cnt) {
                ...
                rtlogger.info("Send rpc verb repair_update_repaired_at for merge table={} replicas={} old_cnt={} new_cnt={}", table_id, replicas, old_cnt, new_cnt);
                ...
                co_await coroutine::parallel_for_each(replicas, [ms = &_messaging, table_id] (const locator::host_id& h) -> future<> {
                    co_await ser::repair_rpc_verbs::send_repair_update_repaired_at(ms, h, table_id);
                });
            }
        }
