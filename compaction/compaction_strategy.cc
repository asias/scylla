...
bool compaction_strategy_impl::worth_dropping_tombstones(const shared_sstable& sst, gc_clock::time_point query_time) {
    ...
    auto gc_before = sst->get_gc_before_for_drop_estimation(query_time);
...
date_tiered_manifest::get_next_sstables(table_state& table_s, std::vector<sstables::shared_sstable>& uncompacting, gc_clock::time_point query_time) {
    if (table_s.get_sstable_set().all()->empty()) {
        return {};
    }

    // Find fully expired SSTables. Those will be included no matter what.
    auto expired = table_s.fully_expired_sstables(uncompacting, query_time);
    ...
    auto compaction_candidates = get_next_non_expired_sstables(table_s, uncompacting, query_time);
    if (!expired.empty()) {
...
std::vector<sstables::shared_sstable>
date_tiered_manifest::get_next_non_expired_sstables(table_state& table_s, std::vector<sstables::shared_sstable>& non_expiring_sstables, gc_clock::time_point query_time) {
    int base = table_s.schema()->min_compaction_threshold();
    int64_t now = get_now(table_s.get_sstable_set().all());
...
compaction_descriptor date_tiered_compaction_strategy::get_sstables_for_compaction(table_state& table_s, std::vector<sstables::shared_sstable> candidates) {
    auto query_time = gc_clock::now();
    auto sstables = _manifest.get_next_sstables(table_s, candidates, query_time);
    ...
    auto e = boost::range::remove_if(candidates, [this, query_time] (const sstables::shared_sstable& sst) -> bool {
        return !worth_dropping_tombstones(sst, query_time);
    });
