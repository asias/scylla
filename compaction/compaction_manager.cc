...
future<compaction_manager::compaction_reenabler>
compaction_manager::stop_and_disable_compaction(compaction_group_view& t) {
    compaction_reenabler cre(*this, t);
...
future<>
compaction_manager::run_with_compaction_disabled(compaction_group_view& t, std::function<future<> ()> func) {
...
future<> compaction_manager::maybe_wait_for_sstable_count_reduction(compaction_group_view& t) {
    ...
    try {
        while (co_await num_runs_for_compaction() > threshold) {
...
future<compaction_manager::compaction_stats_opt> compaction_manager::perform_task_on_all_files(tasks::task_info info, compaction_group_view& t, sstables::compaction_type_options options, owned_ranges_ptr owned_ranges_ptr, get_candidates_func get_func, Args... args) {
    auto gh = start_compaction(t);
    if (!gh) {
        co_return std::nullopt;
    }
...
future<compaction_manager::compaction_stats_opt> compaction_manager::perform_split_compaction(compaction_group_view& t, sstables::compaction_type_options::split opt, tasks::task_info info) {
    ...
    co_return co_await perform_task_on_all_files<split_compaction_task_executor>(info, t, std::move(options), std::move(owned_ranges_ptr), std::move(get_sstables));
}
