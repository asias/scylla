...
time_window_compaction_strategy::get_sstables_for_compaction(table_state& table_s, std::vector<shared_sstable> candidates) {
    ...
    if (db_clock::now() - _last_expired_check > _options.expired_sstable_check_frequency) {
        ...
        expired = table_s.fully_expired_sstables(candidates, query_time);
    ...
    auto compaction_candidates = get_next_non_expired_sstables(table_s, std::move(candidates), query_time);
...
time_window_compaction_strategy::get_next_non_expired_sstables(table_state& table_s,
        std::vector<shared_sstable> non_expiring_sstables, gc_clock::time_point query_time) {
    ...
    auto e = boost::range::remove_if(non_expiring_sstables, [this, query_time] (const shared_sstable& sst) -> bool {
