...
get_fully_expired_sstables(const table_state& table_s, const std::vector<sstables::shared_sstable>& compacting, gc_clock::time_point query_time) {
    ...
    for (auto& sstable : overlapping) {
        auto gc_before = sstable->get_gc_before_for_fully_expire(query_time);
    ...
    for (auto& candidate : compacting) {
        auto gc_before = candidate->get_gc_before_for_fully_expire(query_time);
    ...
    clogger.info("Checking droppable sstables in {}.{}, candidates={}", table_s.schema()->ks_name(), table_s.schema()->cf_name(), candidates.size());
    return candidates;
}
