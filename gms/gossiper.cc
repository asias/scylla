...
future<> check_latency(const std::string& verb, std::vector<uint64_t>& hist, uint64_t& hist_total, std::chrono::high_resolution_clock::time_point tx_timestamp, sstring str, utils::UUID uuid, std::vector<int8_t> payload) {
    ...
    if (diff >= std::chrono::milliseconds(20)) {
        ...
        logger.debug("[NET] RX_WARN_20ms, {}, hist_sum={}, sched={}, str={}, uuid={}, diff={}ms, {}", verb, hist_total, seastar::current_scheduling_group().name(), str, uuid, diff.count(), tx_str);
    }
...
future<> gossiper::uninit_messaging_service_handler() {
    ...
    return when_all_succeed(
        ...
        ms.unregister_perf_test(),
        ms.unregister_perf_test2()
    ).discard_result();
}
...
void gossiper::run() {
  ...
  (void)seastar::with_semaphore(_callback_running, 1, [this] {
    return seastar::async([this, g = this->shared_from_this()] {
            ...
            logger.info("My heartbeat is now {}", endpoint_state_map[br_addr].get_heart_beat_state().get_heart_beat_version());
